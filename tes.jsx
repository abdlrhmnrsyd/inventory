...
function LinktoFolder() {
  ...
  const [isViewingContents, setIsViewingContents] = useState(false); // State untuk mengatur tampilan isi folder
  ...
  const handleGetFolderContents = async (pathToOpen) => {
    ...
    if (path) {
      try {
        ...
        setIsViewingContents(true); // Set state untuk menampilkan isi folder
        Swal.fire(
          "Success",
  ...
  const handleSavePath = () => {
    if (folderPath && folderTitle) {
      const newPath = { path: folderPath, title: folderTitle };
      setSavedPaths([...savedPaths, newPath]);
      setFolderPath("");
      setFolderTitle("");
      Swal.fire("Success", "Folder path saved successfully!", "success");
    } else {
      Swal.fire("Warning", "Please fill in both path and title.", "warning");
    }
  };

  const handleBack = () => {
    setIsViewingContents(false); // Set state untuk kembali ke daftar folder
    setFolderContents([]); // Kosongkan isi folder
  };
    ...
    <div className="p-6 mt-8 bg-gray-800 rounded-lg shadow-lg">
      ...
      {isViewingContents ? (
        <div className="p-4 mt-4 bg-gray-700 rounded-lg">
          <button
            onClick={handleBack}
            className="px-4 py-2 mb-4 text-white transition-colors bg-red-500 rounded-lg shadow-md hover:bg-red-600 hover:shadow-lg"
          >
            Back
          </button>
